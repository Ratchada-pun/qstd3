...
Queue = {
    handleEventClick: function(){
        ...
        $('#tb-waiting tbody').on( 'click', 'tr td a.btn-calling', function (event) {
            ...
            var tr = $(this).closest("tr"), url = $(this).attr("data-url");
            ...
            if(self.checkCounter()){
                swal({
                    ...
                    text: data.pt_name,
                    html: '<small class="text-danger" style="font-size: 13px;">กด Enter เพื่อยืนยัน / กด Esc เพื่อยกเลิก</small>' + '<p>'+countername+'</p>',
                    type: 'question',
                    input: 'radio',
                    inputOptions: {
                        0 : "ต้องการ END ทันที",
                            ...
                            $.ajax({
                                ...
                                success: function(res){
                                    if(res.status == 200){
                                        ...
                                        if(value == 0){
                                            setTimeout(function(){
                                                var tr = $('#tb-calling tr#'+res.model.caller_ids), url = "/app/calling/end-medical";
                                                var key = tr.data("key");
...
$(function() {
    socket.on('register', (res) => {//ออกบัตรคิว
        ...
        if(res.eventOn === 'tb-waiting' && res.state === 'call'){
            ...
            if(res.modelProfile.service_profile_id == modelProfile.service_profile_id && modelForm.counter_service == res.counter.counterserviceid.toString()){
                Queue.reloadTbCalling();//โหลดข้อมูลกำลังเรียก
            }
            swal.close();
...
$('a.activity-callnext').on('click',function(e){
    e.preventDefault();
    var data = dt_tbwaiting.rows(0).data(), url = $(this).attr("data-url");
    var countername = $('#callingform-counter_service').select2('data')[0]['text'] || '';
    if(data.length > 0){
    ...
    if (selectedData.length > 0) {
        if(Queue.checkCounter()){
            ...
            swal({
                ...
                text: "",
                html: '<small class="text-danger" style="font-size: 13px;">กด Enter เพื่อยืนยัน / กด Esc เพื่อยกเลิก</small><br><p>'+ queNumber.join(", ") + '</p><p>'+countername+'</p>',
                type: 'question',
                input: 'radio',
                inputOptions: {
                    0 : "ต้องการ END ทันที",
