...
use frontend\modules\app\models\RegisterSearch;
...
class RegisterController extends Controller
{
    ...
    {
        // print_r(date("d/m/") .'/' .(date("Y") + 543));
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        if (isset($_GET['password'])) {
            $password = $_GET['password'];
            if ($password === '@andaman888') {
                $model = Register::find()
                    ->where(['UpdateDate' => date("d/m/") . (date("Y") + 543)])
                    ->orWhere(['UpdateDate' => date("d/m/Y")])
