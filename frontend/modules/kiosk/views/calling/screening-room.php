                    ...
                    <div class="panel-body" style="padding-buttom: 0px;">
                                            ...
                                            'pluginEvents' => [
                                                "change" => "function() {
                                                    if($(this).val() != '' && $(this).val() != null){
                                                        location.replace(baseUrl + \"/kiosk/calling/screening-room?secid=\" + $(this).val());
                                                    }else{
                                                        location.replace(baseUrl + \"/kiosk/calling/screening-room?secid=null\");
                                                    }
                                                }",
                                            ]
                                        ]);
                                                ...
                                                'pluginEvents' => [
                                                    "change" => "function() {
                                                        ...
                                                        if(counter !== null && section !== null){
                                                            location.replace(baseUrl + \"/kiosk/calling/screening-room?secid=\" + section + \"&counterid=\" + $(this).val());
                                                        }else{
                                                            location.replace(baseUrl + \"/kiosk/calling/screening-room?secid=null&counterid=null\");
                                                        }
                                                    }",
                                                ]
                                            ],
                                            ...
                                            'beforeHeader' => [
                                                [
                                                    'columns' => [
                                                        ...
                                                        ['content' => 'โต๊ะ','options' => ['style' => 'text-align: center;']],
                        ...
                        <div class="hpanel">
                            ...
                            echo Tabs::widget([
                                'items' => [
                                    [
                                        'label' => 'รอเรียกคัดกรอง '. Html::tag('span','0',['id' => 'count-waiting','class' => 'badge']),
                                        'active' => true,
...
<?= Datatables::widget([
    ...
    'clientOptions' => [
        'ajax' => [
            'url' => Url::base(true).'/kiosk/calling/data-tbwaiting-sr',
            'data' => [
                'secid' => $model['section'],
        ...
        'columns' => [
            ...
            ["data" => "counter_service_id","className" => "dt-body-center dt-head-nowrap","title" => "โต๊ะ"],
...
<?= Datatables::widget([
    ...
    'clientOptions' => [
        'ajax' => [
            'url' => Url::base(true).'/kiosk/calling/data-tbcalling-sr',
            'data' => [
                'secid' => $model['section'],
                'counter' => $model['counter'],
            ]
        ...
        'columns' => [
            ...
            ["data" => "pt_name","className" => "dt-body-left dt-head-nowrap","title" => "<i class=\"fa fa-user\"></i> ชื่อ-นามสกุล"],
            ["data" => "pt_visit_type","className" => "dt-body-center dt-head-nowrap","title" => "ประเภท"],
            ["data" => "counterservice_name","className" => "dt-body-center dt-head-nowrap","title" => "โต๊ะ"],
            ["data" => "checkin_date","className" => "dt-body-center dt-head-nowrap","title" => "<i class=\"fa fa-clock-o\"></i> เวลามาถึง"],
            ["data" => "service_status_name","className" => "dt-body-center dt-head-nowrap","title" => "สถานะ"],
            ["data" => "actions","className" => "dt-center dt-nowrap","orderable" => false,"title" => "<i class=\"fa fa-cogs\"></i> ดำเนินการ"]
        ],
...
<?= Datatables::widget([
    ...
    'clientOptions' => [
        'ajax' => [
            'url' => Url::base(true).'/kiosk/calling/data-tbhold-sr',
            'data' => [
                'secid' => $model['section'],
                'counter' => $model['counter'],
            ]
...
$(function() {
    ...
    .on('call-screening-room', (res) => {//เรียกคิวคัดกรอง /kiosk/calling/screening-room
        if(res.eventOn === 'tb-waiting'){//เรียกคิวจาก table รอเรียกคัดกรอง
    ...
    .on('hold-screening-room', (res) => {//Hold คิวคัดกรอง /kiosk/calling/screening-room
        dt_tbcalling.ajax.reload();//โหลดข้อมูลกำลังเรียกใหม่
        dt_tbhold.ajax.reload();//โหลดข้อมูลพักคิวใหม่
    })
    .on('endq-screening-room', (res) => {//End คิวคัดกรอง /kiosk/calling/screening-room
        dt_tbcalling.ajax.reload();//โหลดข้อมูลกำลังเรียกใหม่
        dt_tbhold.ajax.reload();//โหลดข้อมูลพักคิวใหม่
    });
});
...
$('#tb-waiting tbody').on( 'click', 'tr td a.btn-calling', function (event) {
    ...
    swal({
        ...
        if (result.value) {//Confirm
            $.ajax({
                method: "POST",
                url: baseUrl + "/kiosk/calling/call-screening-room",
                dataType: "json",
                data: {
...
$('#tb-calling tbody').on( 'click', 'tr td a.btn-recall', function (event) {
    ...
    swal({
        ...
        if (result.value) {//Confirm
            $.ajax({
                method: "POST",
                url: baseUrl + "/kiosk/calling/recall-screening-room",
                dataType: "json",
                data: {
                    data:data,//Data in column Datatable
                    model:model//Data Model CallingForm {section, counter}
...
$('#tb-calling tbody').on( 'click', 'tr td a.btn-hold', function (event) {
    ...
    swal({
        ...
        if (result.value) {//Confirm
            $.ajax({
                method: "POST",
                url: baseUrl + "/kiosk/calling/hold-screening-room",
                dataType: "json",
                data: {
                    data:data,//Data in column Datatable
                    model:model//Data Model CallingForm {section, counter}
...
$('#tb-hold tbody').on( 'click', 'tr td a.btn-calling', function (event) {
    ...
    swal({
        ...
        if (result.value) {//Confirm
            $.ajax({
                method: "POST",
                url: baseUrl + "/kiosk/calling/callhold-screening-room",
                dataType: "json",
                data: {
                    data:data,//Data in column Datatable
                ...
                success: function(res){
                    if(res.status == 200){//success
                        dt_tbcalling.ajax.reload();//โหลดข้อมูลกำลังเรียกใหม่
                        dt_tbhold.ajax.reload();//โหลดข้อมูลพักคิวใหม่
                        //$('li.tab-calling,#tab-calling').addClass('active');//แสดงข้อมูลแท็บ "กำลังเรียก"
                        //$('li.tab-hold, #tab-hold').removeClass('active');//ซ่อนข้อมูลแท็บ "พักคิว"
                        toastr.success('CALL ' + data.qnumber, 'Success!', {timeOut: 3000,positionClass: "toast-top-right",});
                        socket.emit('call-screening-room', res);//sending data
                    }else{//error
                        swal({
...
$('#tb-hold tbody').on( 'click', 'tr td a.btn-end', function (event) {
    ...
    swal({
        ...
        if (result.value) {//Confirm
            $.ajax({
                method: "POST",
                url: baseUrl + "/kiosk/calling/endhold-screening-room",
                dataType: "json",
                data: {
                    data:data,//Data in column Datatable
                    model:model//Data Model CallingForm
...
qFunc = {
    ...
    setSectionFormValue: function(secid){
        $.ajax({
            method: "POST",
            url: "/kiosk/calling/set-section-formvalue",
            data: {secid: secid},
            dataType: "json",
            success: function(res){
                console.log(model);
