...
<?php $form = ActiveForm::begin([
    'id' => 'form-counter-type', 'type' => ActiveForm::TYPE_HORIZONTAL, 
    ...
    <?php DynamicFormWidget::begin([
        ...
        'model' => $models[0],
        'formId' => 'form-counter-type',
        'formFields' => [
            'tb_counterservice_typeid',
                    ...
                    <div class="panel-body">
                        <?php
                            if (! $model->isNewRecord) {
                                echo Html::activeHiddenInput($model, "[{$index}]tb_counterservice_typeid");
                            }
                        ?>
                        <div class="form-group">
                            <?= Html::activeLabel($model, "[{$index}]tb_counterservice_type", ['label' => 'ชื่อประเภท','class'=>'col-sm-2 control-label']) ?>
                            <div class="col-sm-8">
                                <?= $form->field($model, "[{$index}]tb_counterservice_type",['showLabels'=>false])->textInput([
                        ...
                        <div class="form-group">
                            <?= Html::activeLabel($model, "[{$index}]q_waiting_status", ['class'=>'col-sm-2 control-label']) ?>
                            <div class="col-sm-4">
                                <?=
                                $form->field($model, "[{$index}]q_waiting_status")->widget(Select2::classname(), [
                                    'data' => ArrayHelper::map(TbServiceStatus::find()->asArray()->all(),'service_status_id','service_status_name'),
                            ...
                            <?= Html::activeLabel($model, "[{$index}]q_calling_status", ['class'=>'col-sm-2 control-label']) ?>
                            <div class="col-sm-4">
                                <?=
                                $form->field($model, "[{$index}]q_calling_status")->widget(Select2::classname(), [
                                    'data' => ArrayHelper::map(TbServiceStatus::find()->asArray()->all(),'service_status_id','service_status_name'),
                                    'options' => ['placeholder' => 'เลือกสถานะ...'],
                                    'pluginOptions' => [
                                        'allowClear' => true
...
//Form Event
var table = $('#tb-counter-type').DataTable();
var \$form = $('#form-counter-type');
