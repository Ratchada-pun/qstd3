...
<?php $form = ActiveForm::begin([
    'id' => 'form-visit-type', 'type' => ActiveForm::TYPE_HORIZONTAL, 
                    ...
                    <div class="panel-body">
                        <?php
                            if (! $model->isNewRecord) {
                                echo Html::activeHiddenInput($model, "[{$index}]pt_visit_type_id");
                            }
                        ?>
                        <div class="form-group">
                            <?= Html::activeLabel($model, "[{$index}]pt_visit_type", ['label' => 'ชื่อประเภท','class'=>'col-sm-2 control-label']) ?>
                            <div class="col-sm-8">
                                <?= $form->field($model, "[{$index}]pt_visit_type",['showLabels'=>false])->textInput([
                                    'placeholder' => 'ชื่อประเภท'
                                ]); ?>
                            </div>
                        </div>
                        <div class="form-group">
                            <?= Html::activeLabel($model, "[{$index}]pt_visit_type_prefix", ['label' => 'ตัวอักษร/ตัวเลข นำหน้าคิว','class'=>'col-sm-2 control-label']) ?>
                            <div class="col-sm-4">
                                <?= $form->field($model, "[{$index}]pt_visit_type_prefix",['showLabels'=>false])->textInput([
                                    'placeholder' => 'ตัวอักษร/ตัวเลข นำหน้าคิว'
                                ]); ?>
                            </div>
                        </div>
                        <div class="form-group">
                            <?= Html::activeLabel($model, "[{$index}]pt_visit_type_digit", ['label' => 'จำนวนหลักหมายเลขคิว','class'=>'col-sm-2 control-label']) ?>
                            <div class="col-sm-4">
                                <?= $form->field($model, "[{$index}]pt_visit_type_digit",['showLabels'=>false])->textInput([
...
//Form Event
var table = $('#tb-visit-type').DataTable();
var \$form = $('#form-visit-type');
