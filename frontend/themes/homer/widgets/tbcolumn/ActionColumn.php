...
class ActionColumn extends BaseObject
{
    ...
    protected function setDefaultButton($name, $title, $icon)
    {
        ...
        $this->buttons[$name] = function ($url) use ($name, $title, $icon) {
            ...
            if ($name === 'delete') {
                $item = isset($this->grid->itemLabelSingle) ? $this->grid->itemLabelSingle : Yii::t('yii', 'item');
                $options['data-method'] = 'post';
                $options['data-confirm'] = Yii::t('yii', 'Are you sure to delete this {item}?', ['item' => $item]);
            }
