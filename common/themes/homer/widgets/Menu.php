...
class Menu extends Widget
{
     ...
     * This property will be overridden by the `template` option set in individual menu items via [[items]].
     */
    public $labelTemplate = '<span class="nav-label">{icon} {label}</span>';
    /**
    ...
    {
        if (!empty($item['items'])) {
            $linkTemplate = '<a href="{url}"><span class="nav-label">{label}</span><span class="fa arrow"></span></a>';
        }else{
        ...
        if (isset($item['url'])) {
            ...
            return strtr($template, [
                ...
                '{label}' => $item['label'],
                '{icon}' => isset($item['icon']) ? Html::tag('i','',['class' => 'fa fa-'.$item['icon']]) : '',
            ]);
        }
